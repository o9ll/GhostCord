...
// Guard: check that an item is a complete Message object.
// Discord may include raw snowflakes (strings/numbers) in payload.messages
// to represent not-yet-loaded lazy-loading messages.
...
export default definePlugin({
    ...
    coolReAddDeletedMessages: (messages: LoggedMessageJSON[] & { extra: LoggedMessageJSON[]; }, payload: LoadMessagePayload) => {
        try {
            ...
            // Discord may include raw snowflakes (strings) in this array for
            // partial/unloaded messages. A single non-object element crashes Discord
            // later with "Cannot use 'in' operator to search for 'flags' in <id>".
            // Always filter, even when there are no deleted messages to reinject
            // (this is the most common case, hence the most common bug).
