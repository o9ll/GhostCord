...
// Guard: check that an item is a Message object (not a raw snowflake string/bigint).
// Discord may store raw IDs in the message array (partial messages, references),
...
export function reAddDeletedMessages(messages: LoggedMessageJSON[], deletedMessages: LoggedMessageJSON[], channelStart: boolean, channelEnd: boolean) {
    ...
    // Separate valid Message objects from raw snowflakes/primitives Discord may inject
    ...
    // Modify the original array in-place to avoid breaking Webpack references.
    // Raw elements (snowflakes) are reinserted at the end to preserve Discord's expectations.
    messages.length = 0;
