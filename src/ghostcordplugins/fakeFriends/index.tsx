...
const userContextPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    try {
        ...
        if (!state) {
            ...
            items = [
                <Menu.MenuItem key="ff-remove" id="ff-remove" label="Remove fake friend" color="danger"
                    action={async () => {
                        fakeState.delete(userId);
                        await persistState();
                        FluxDispatcher.dispatch({ type: "RELATIONSHIP_REMOVE", relationship: { id: userId } });
...
const guildContextPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    try {
        ...
        // Count how many fake states belong to this server
        const memberIds = new Set<string>(GuildMemberStore.getMemberIds(guildId) as string[]);
        ...
        // "Remove fake friend requests" button — visible only if fakes exist for this server
        if (fakeCount > 0) {
            items.push(
                <Menu.MenuItem
                    key="ff-g-remove"
