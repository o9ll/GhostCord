...
const DS_KEY = "ghostcord-ai-history";
...
function findFriend(name: string): { id: string; username: string; } | null {
    ...
    } catch (e) { console.warn("[GhostcordAI] findFriend:", e); }
    return null;
}
...
async function callUser(userId: string): Promise<void> {
    // Open DM and navigate to it first
    ...
    // Method 2: CALL_CONNECT dispatch (starts a call on existing DM channel)
    try {
    ...
    // Method 3: navigate to DM channel and dispatch RING
    FluxDispatcher?.dispatch({
        type: "CALL_CREATE",
        channelId,
...
function joinVoiceChannel(name: string): void {
    const query = name.toLowerCase().trim();
    // Extract only numbers/words from name (ignore mentioned server)
    // ex: "222 on shibuya" → search for "222" only
    ...
    function matchesChannel(channelName: string): boolean {
        ...
        return cn.includes(queryWords) || cn.includes(query) ||
            // Partial match: each query word in name
    ...
    } catch (e) { console.warn("[GhostcordAI] joinVoiceChannel guild search:", e); }
    ...
    // List available channels in error for debugging
...
// detectAction is now merged into callAI to save one API request
...
function GhostcordAIChat({ rootProps, panelMode, initialMessage }: { rootProps?: any; panelMode?: boolean; initialMessage?: string; }) {
    ...
    async function send(overrideText?: string | React.MouseEvent) {
        ...
        try {
            ...
            // Detect Discord actions AND generate response in 1 call
            // (instead of 2 separate calls like before — saves 50% quota)
            let reply: string;
                ...
                <div className="nai-header-left">
                    ...
                    <div className="nai-header-info">
                        <div className="nai-header-title-row">
                            <span className="nai-header-title">{t("Ghostcord AI")}</span>
                    ...
                    {messages.length === 0 ? (
                        <div className="nai-empty">
                            ...
                            <p className="nai-empty-sub">
                                {hasKey ? t("Ask anything!") : t("Configure your API key in Ghostcord Settings > Plugins > GhostcordAI")}
                            </p>
                            <div className="nai-chips">
                                {hasKey
                                    ...
                                    : <button className="nai-chip nai-chip--link" onClick={() => showApiKeyWarning("GhostcordAI")}>{t("🔑 Groq Key (free)")}</button>
                                }
                            </div>
                        </div>
                                ...
                                <div className="nai-msg-body">
                                    {!grouped && (
                                        <div className="nai-msg-meta">
                                            <span className="nai-msg-author">{msg.role === "user" ? (UserStore.getCurrentUser()?.globalName ?? UserStore.getCurrentUser()?.username ?? "You") : "Ghostcord AI"}</span>
...
export function GhostcordAIPanel() {
    return <GhostcordAIChat panelMode={true} />;
}

// ── Ghostcord AI button in DM panel (replaces Shop) ─────────────────

function GhostcordAINavButton({ selected }: { selected?: boolean; }) {
    const handleClick = () => openModal(p => <GhostcordAIChat rootProps={p} />);
    return (
        <div className={`nai-nav-item ${selected ? "selected" : ""}`} role="button" tabIndex={0}
            ...
            <span className="nai-nav-label">{t("Ghostcord AI")}</span>
...
export default definePlugin({
    ...
    patches: [
        {
            // Patch 1: Replace Shop page with our GhostcordAI panel
        ...
        {
            // Patch 2: Inject GhostcordAI button into DM sidebar (Old system re-enabled with version fix)
            find: ".FRIENDS},\"friends\"",
            replacement: {
                // Target the Shop button injection in the Sidebar component
                // $1 captures the selection expression (selected: ...)
    ...
    start() {
        // Auto-migration: copy Settings key → DataStore on first run
        ...
        // DOM fallback system if Webpack patch fails on this Discord version
        ...
        const inject = () => {
            ...
            if (createRoot) {
                ...
                this._reactRoot.render(<GhostcordAINavButton />);
            } else {
    ...
    renderNavButton(selected?: boolean) {
        return <GhostcordAINavButton selected={selected} />;
    },

    renderPanel() {
        return <GhostcordAIPanel />;
    },
    ...
    toolboxActions: {
        "Ghostcord AI"() {
            openModal(props => <GhostcordAIChat rootProps={props} />);
        },
    },
});
