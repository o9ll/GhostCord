...
// FIX DM SCROLL CRASH: msgCache reduced from 8000 to 3000 entries, purge from 1000 to 500
// The brutal purge of 1000 entries at once during scroll caused a synchronous
// work spike that blocked the main thread at the critical re-render moment.
// Smaller cache + smaller purge = less impact during scroll.
...
function cacheMsg(msg: any) {
    if (!msg?.id) return;
    // FIX: Don't purge cache during message loading (DM scroll)
    // Purging in the middle of LOAD_MESSAGES_SUCCESS forced a recalculation of
...
const CFG: Record<LogType, { label: string; color: string; }> = {
    ...
    guild_timeout: { label: t("Timed Out"), color: "#faa61a" },
...
function renderContent(text: string) {
    if (!text) return text;
    // Replace <@ID> or <@!ID> pings with @name
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Ghostcord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `ghostcord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
function subscribeToEvents() {
    ...
    sub("LOAD_MESSAGES_SUCCESS", d => {
        if (!d) return;
        // FIX CRASH DM SCROLL: isLoadingMessages blocks the msgCache purge during
        ...
        if (typeof requestIdleCallback !== "undefined") {
            requestIdleCallback(() => {
                ...
                // Deferred purge if cache exceeds the limit
                pruneMsgCache();
            }, { timeout: 3000 });
        } else {
            setTimeout(() => {
    ...
    // GUILD_MEMBER_LIST_UPDATE: dispatched to all members when the list changes
    // DELETE ops = someone left the server (visible without admin perms)
    ...
    // Capture logout/disconnect (partial as the plugin stops on full disconnect)
    sub("LOGOUT", () => { pushLog({ type: "user_disconnect", content: t("Disconnected from account"), authorName: "System" }); });
}
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("ghostcord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    ...
    stop() {
        ...
        removeHeaderBarButton("ghostcord-event-logs");
