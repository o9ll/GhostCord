...
const DoubleCallButton: UserAreaButtonFactory = ({ iconForeground }: UserAreaRenderProps) => {
    ...
    async function handleClick() {
        ...
        // Multi-listen: stream WAV from ghost-server
    ...
    return (
        <UserAreaButton
            ...
            icon={<PhoneIcon className={`${iconForeground} ${active ? "ghostcord-dc-active" : ""}`} />}
            onClick={handleClick}
        />
    );
};
            ...
            if (proto && !proto.__dc_patched) {
                ...
                proto.format = function (args: any) {
                    ...
                    if (anchorState && typeof res === "string" &&
                        (res.toLowerCase().includes("disconnected") || res.toLowerCase().includes("location")) &&
                        (res.includes("other") || res.includes("location") || res.includes("another"))) {
