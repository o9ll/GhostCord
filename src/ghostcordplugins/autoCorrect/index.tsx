...
async function correctText(text: string): Promise<string> {
    ...
    try {
        const corrected = await groqChat({
            ...
            temperature: 0,
            maxTokens: 512,
            // Force a lightweight model for correction — save the 70B quota for AI
            forceModel: "llama-3.1-8b-instant",
        });

        if (!corrected || corrected.trim() === "" || corrected === text) return text;

        // Safety guard against infinite repetition or hallucinations
        ...
        return corrected.replace(/^"(.*)"$/, "$1").trim(); // Strip any surrounding quotes
    } catch (e: any) {
...
const AutoCorrectChatBarButton: ChatBarButtonFactory = ({ type }) => {
    ...
    const toggle = async () => {
        if (!enabled) {
            // Check API key is configured before enabling
...
export default definePlugin({
    ...
    description: "Automatically corrects spelling and grammar before sending. Requires a free Groq API key configured in GhostcordAI.",
