...
const settings = definePluginSettings({
    ...
    syncWithFriends: {
        ...
        description: "Sync wallpapers with Ghostcord friends in DMs.",
        default: true,
        restartNeeded: false,
    },
    vpsUrl: {
...
function applyWallpaper(channelId?: string) {
    ...
    if (!document.getElementById(STYLE_ID)) {
        ...
        style.id = STYLE_ID;
        style.textContent = `
/* Messages area: make background transparent to show wallpaper */
...
/* Ensure parent is relative for background positioning */
[class*="messagesWrapper"],
[class*="chatContent"] {
...
// Right-click on user → find DM channel for this specific user
const userContextMenuPatch: NavContextMenuPatchCallback = (children, { user }: any) => {
    if (!user?.id) return;
    // Resolve DM channel for this user (not the current channel!)
