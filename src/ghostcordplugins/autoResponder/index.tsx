...
import { getGroqKey, groqChat } from "../ghostcordAI/groqManager";
            ...
            <div style={{
                ...
                <div>
                    ...
                    <div style={{ fontSize: "13px", marginTop: "4px" }}>
                        ...
                        Please configure it once in the <strong>GhostcordAI</strong> settings.
                    </div>
                </div>
            </div>
        )
...
async function handleMessage(message: any) {
    ...
    // STRICT RESTRICTION: DMs only (Type 1)
        ...
        if (!apiKey) {
            try {
                ...
                openConfirmationModal({
                    ...
                    content: "AutoResponder requires a Groq API Key to function. Please configure it once in the GhostcordAI settings.",
                    confirmText: "Configure GhostcordAI",
                    cancelText: "Cancel",
                    onConfirm: () => {
                        const { openModal } = findByPropsLazy("openModal");
                        // Logic to open GhostcordAI settings if possible
                    }
                });
            } catch (e) {
...
1. SHORT REPLIES : Give concise responses (1-2 sentences max). Don't write long paragraphs.
2. DISCRETION : Only use my personal info (e.g., Paris) when relevant. Don't bring everything back to Paris every time.
3. NATURAL WRITING : In text, don't say "uh..." or "wait" when thinking. Just give the result or continue the sentence. Remove all traces of verbal hesitation.
4. HUMAN : Talk like a Discord friend (light SMS language ok if my style allows it).

MISSION :
Reply naturally. RETURN ONLY THE TEXT OF YOUR REPLY.`;

        const reply = await groqChat({
            messages: [
                { role: "system", content: "You are a highly customizable AutoResponder for Discord." },
        ...
        if (reply && !reply.startsWith("❌")) {
            // Realistic delay: base fixed + proportional to message length
                                ...
                                if (!key) {
                                    openModal(props2 => (
                                        <ModalRoot {...props2} size={ModalSize.SMALL}>
                                            ...
                                            <ModalContent>
                                                <Text variant="text-md/normal" style={{ marginBottom: 16 }}>
                                                    AutoResponder requires a Groq API Key to function. Please configure it once in the GhostcordAI settings.
                                                </Text>
                                            </ModalContent>
                                            <div style={{ padding: "16px", display: "flex", justifyContent: "flex-end", gap: "8px" }}>
