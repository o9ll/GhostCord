...
// ─────────────────────────────────────────────────────────────────────────────
// Token cache — capture tokens from native Discord accounts
// ─────────────────────────────────────────────────────────────────────────────
...
/** Switch for native accounts without token — uses Discord native mechanism */
function switchNativeAccount(userId: string) {
    try {
        ...
        // Fallback: dispatch flux event as Discord does natively
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("ghostcord-mi-ctx-root") ?? document.createElement("div");
        el.id = "ghostcord-mi-ctx-root";
    ...
    // createPortal mounts menu in document.body — outside modal DOM
    // which bypasses z-index and overflow of ModalRoot
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("ghostcord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("ghostcord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("ghostcord-mi-ctx-root");
        if (root) root.remove();
    },
});
